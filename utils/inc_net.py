    ...
    def forward(self, x):
        ...
        if self.bias_correction:
            ...
            for i, layer in enumerate(self.bias_layers):
                logits = layer(logits, sum(
                    self.task_sizes[:i]), sum(self.task_sizes[:i+1]))
            out['logits'] = logits
        out.update(x)
        return out
...
class FOSTERNet(nn.Module):
    ...
    ##  self.up_cam = self._compute_cam(self.up_feature_map, self.up_classifier[0].weight)
     #   out.update({"out_up1": self.up_out})
      #  out.update({"out_up2": self.up_cam})
